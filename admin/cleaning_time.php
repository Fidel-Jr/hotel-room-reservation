<?php 
    ...
    $sql = "SELECT booking.*, booking.id AS booking_id, room.id, room.room_number, room_type.room_type, user.email FROM booking INNER JOIN room ON booking.room_id = room.id INNER JOIN room_type ON room_type.id = room.room_type INNER JOIN user ON booking.user_id = user.id HAVING booking.check_out = '$currentDate' AND '$currentTime' >= booking.check_out_time AND '$currentTime' <='14:30:00' AND booking.booking_status = 'Cleaning' ";
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td><a href="../backend/room_cleaned.php?id=<?php echo $row['booking_id']; ?>" class="btn btn-sm btn-success">Done</a></td>
                            </tr>
                    <?php
                        }
